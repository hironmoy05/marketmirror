...
export const slice = createSlice({
    ...
    reducers: {
        ...
        bugResolved: (bug, action) => {
            const index = bug.userStore.findIndex(bug => bug.id === action.payload.id);
        ...
        projectResolved: (bug, action) => {
            const index = bug.userStore.findIndex(pro => pro.id === action.payload.id)
            bug.userStore[index].isTrue = true;
        }
    }
});

export const {bugAdded, bugResolved, bugAssignToUser, getUserId, removeUserId, projectAdded, projectResolved, bugReceived} = slice.actions;
